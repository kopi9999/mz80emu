#include "execute_arit.h"
#include "execute_control.h"

enum Error add_a_r(struct Instance *__restrict i, void **__restrict inf) {
  uint8_t carries = 0;
  switch (i->instruction & 0b00000111) {
  case A: carries = getAddCarries(i->A, i->A, 0); i->A += i->A; break;
  case B: carries = getAddCarries(i->A, i->B, 0); i->A += i->B; break;
  case C: carries = getAddCarries(i->A, i->C, 0); i->A += i->C; break;
  case D: carries = getAddCarries(i->A, i->D, 0); i->A += i->D; break;
  case E: carries = getAddCarries(i->A, i->E, 0); i->A += i->E; break;
  case H: carries = getAddCarries(i->A, i->H, 0); i->A += i->H; break;
  case L: carries = getAddCarries(i->A, i->L, 0); i->A += i->L; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = 0;
  if (i->A == 0) {flags |= 0b01000000;} //Z flag
  if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
  if (carries & 0b00001000) {flags |= 0b00010000;} //H flag
  if (carries & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
  i->F = flags;
  return nop(i, inf);
  }  

enum Error add_a_n(struct Instance *__restrict i, void **__restrict inf) {
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    *(uint16_t*) inf[0] = i->PC; //addr
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->PC += 1;
    uint8_t carries = getAddCarries(i->A, i->tmp, 0);
    i->A += i->tmp;

    uint8_t flags = 0;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (carries & 0b00001000) {flags |= 0b00010000;} //H flag
    if (carries & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}  

enum Error add_a_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    uint8_t carries = getAddCarries(i->A, i->tmp, 0);
    i->A += i->tmp;

    uint8_t flags = 0;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (carries & 0b00001000) {flags |= 0b00010000;} //H flag
    if (carries & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}

// ADC

enum Error adc_a_r(struct Instance *__restrict i, void **__restrict inf) {
  uint8_t carries = 0;
  uint8_t carry = i->F & 0b00000001;
  switch (i->instruction & 0b00000111) {
  case A: carries = getAddCarries(i->A, i->A, carry); i->A += i->A + carry; break;
  case B: carries = getAddCarries(i->A, i->B, carry); i->A += i->B + carry; break;
  case C: carries = getAddCarries(i->A, i->C, carry); i->A += i->C + carry; break;
  case D: carries = getAddCarries(i->A, i->D, carry); i->A += i->D + carry; break;
  case E: carries = getAddCarries(i->A, i->E, carry); i->A += i->E + carry; break;
  case H: carries = getAddCarries(i->A, i->H, carry); i->A += i->H + carry; break;
  case L: carries = getAddCarries(i->A, i->L, carry); i->A += i->L + carry; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = 0;
  if (i->A == 0) {flags |= 0b01000000;} //Z flag
  if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
  if (carries & 0b00001000) {flags |= 0b00010000;} //H flag
  if (carries & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
  i->F = flags;
  return nop(i, inf);
  }  

enum Error adc_a_n(struct Instance *__restrict i, void **__restrict inf) {
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    *(uint16_t*) inf[0] = i->PC; //addr
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->PC += 1;
    uint8_t carry = i->F & 0b00000001;
    uint8_t carries = getAddCarries(i->A, i->tmp, carry);
    i->A += i->tmp + carry;

    uint8_t flags = 0;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (carries & 0b00001000) {flags |= 0b00010000;} //H flag
    if (carries & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}  

enum Error adc_a_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    uint8_t carry = i->F & 0b00000001;
    uint8_t carries = getAddCarries(i->A, i->tmp, carry);
    i->A += i->tmp + carry;

    uint8_t flags = 0;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (carries & 0b00001000) {flags |= 0b00010000;} //H flag
    if (carries & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}

enum Error sub_r(struct Instance *__restrict i, void **__restrict inf) {
  uint8_t borrows = 0;
  switch (i->instruction & 0b00000111) {
  case A: borrows = getSubtractBorrows(i->A, i->A, 0); i->A -= i->A; break;
  case B: borrows = getSubtractBorrows(i->A, i->B, 0); i->A -= i->B; break;
  case C: borrows = getSubtractBorrows(i->A, i->C, 0); i->A -= i->C; break;
  case D: borrows = getSubtractBorrows(i->A, i->D, 0); i->A -= i->D; break;
  case E: borrows = getSubtractBorrows(i->A, i->E, 0); i->A -= i->E; break;
  case H: borrows = getSubtractBorrows(i->A, i->H, 0); i->A -= i->H; break;
  case L: borrows = getSubtractBorrows(i->A, i->L, 0); i->A -= i->L; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = 0b00000010;
  if (i->A == 0) {flags |= 0b01000000;} //Z flag
  if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
  if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
  if (borrows & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
  i->F = flags;
  return nop(i, inf);
  }  

enum Error sub_n(struct Instance *__restrict i, void **__restrict inf) {
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    *(uint16_t*) inf[0] = i->PC; //addr
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->PC += 1;
    uint8_t borrows = getSubtractBorrows(i->A, i->tmp, 0);
    i->A -= i->tmp;

    uint8_t flags = 0b00000010;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
    if (borrows & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}  

enum Error sub_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    uint8_t borrows = getSubtractBorrows(i->A, i->tmp, 0);
    i->A -= i->tmp;

    uint8_t flags = 0b00000010;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
    if (borrows & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}

enum Error sbc_a_r(struct Instance *__restrict i, void **__restrict inf) {
  uint8_t borrows = 0;
  uint8_t borrow = i->F & 0b00000001;
  switch (i->instruction & 0b00000111) {
  case A: borrows = getSubtractBorrows(i->A, i->A, borrow); i->A -= i->A - borrow; break;
  case B: borrows = getSubtractBorrows(i->A, i->B, borrow); i->A -= i->B - borrow; break;
  case C: borrows = getSubtractBorrows(i->A, i->C, borrow); i->A -= i->C - borrow; break;
  case D: borrows = getSubtractBorrows(i->A, i->D, borrow); i->A -= i->D - borrow; break;
  case E: borrows = getSubtractBorrows(i->A, i->E, borrow); i->A -= i->E - borrow; break;
  case H: borrows = getSubtractBorrows(i->A, i->H, borrow); i->A -= i->H - borrow; break;
  case L: borrows = getSubtractBorrows(i->A, i->L, borrow); i->A -= i->L - borrow; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = 0b00000010;
  if (i->A == 0) {flags |= 0b01000000;} //Z flag
  if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
  if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
  if (borrows & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
  i->F = flags;
  return nop(i, inf);
  }  

enum Error sbc_a_n(struct Instance *__restrict i, void **__restrict inf) {
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    *(uint16_t*) inf[0] = i->PC; //addr
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->PC += 1;
    uint8_t borrow = i->F & 0b00000001;
    uint8_t borrows = getSubtractBorrows(i->A, i->tmp, borrow);
    i->A -= i->tmp;

    uint8_t flags = 0b00000010;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
    if (borrows & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}  

enum Error sbc_a_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    uint8_t borrow = i->F & 0b00000001;
    uint8_t borrows = getSubtractBorrows(i->A, i->tmp, borrow);
    i->A -= i->tmp;

    uint8_t flags = 0b00000010;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
    if (borrows & 0b10000000) {flags |= 0b00000101;} // C and P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}

enum Error and_r(struct Instance *__restrict i, void **__restrict inf) {
  i->tmp = i->A;
  switch (i->instruction & 0b00000111) {
  case A: i->A &= i->A; break;
  case B: i->A &= i->B; break;
  case C: i->A &= i->C; break;
  case D: i->A &= i->D; break;
  case E: i->A &= i->E; break;
  case H: i->A &= i->H; break;
  case L: i->A &= i->L; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = 0b00010000;
  if (i->A == 0) {flags |= 0b01000000;} //Z flag
  if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
  if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
  i->F = flags;
  return nop(i, inf);
  }  

enum Error and_n(struct Instance *__restrict i, void **__restrict inf) {
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    *(uint16_t*) inf[0] = i->PC; //addr
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->PC += 1;
    i->A &= i->tmp;

    uint8_t flags = 0b00010000;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}  

enum Error and_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->A &= i->tmp;

    uint8_t flags = 0b00000010;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}

enum Error or_r(struct Instance *__restrict i, void **__restrict inf) {
  switch (i->instruction & 0b00000111) {
  case A: i->A |= i->A; break;
  case B: i->A |= i->B; break;
  case C: i->A |= i->C; break;
  case D: i->A |= i->D; break;
  case E: i->A |= i->E; break;
  case H: i->A |= i->H; break;
  case L: i->A |= i->L; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = 0;
  if (i->A == 0) {flags |= 0b01000000;} //Z flag
  if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
  if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
  i->F = flags;
  return nop(i, inf);
  }  

enum Error or_n(struct Instance *__restrict i, void **__restrict inf) {
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    *(uint16_t*) inf[0] = i->PC; //addr
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->PC += 1;
    i->A |= i->tmp;

    uint8_t flags = 0;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}  

enum Error or_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->A |= i->tmp;

    uint8_t flags = 0;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}

enum Error xor_r(struct Instance *__restrict i, void **__restrict inf) {
  i->tmp = i->A;
  switch (i->instruction & 0b00000111) {
  case A: i->A ^= i->A; break;
  case B: i->A ^= i->B; break;
  case C: i->A ^= i->C; break;
  case D: i->A ^= i->D; break;
  case E: i->A ^= i->E; break;
  case H: i->A ^= i->H; break;
  case L: i->A ^= i->L; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = 0;
  if (i->A == 0) {flags |= 0b01000000;} //Z flag
  if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
  if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
  i->F = flags;
  return nop(i, inf);
  }  

enum Error xor_n(struct Instance *__restrict i, void **__restrict inf) {
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    *(uint16_t*) inf[0] = i->PC; //addr
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->PC += 1;
    i->A ^= i->tmp;

    uint8_t flags = 0;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}  

enum Error xor_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->A ^= i->tmp;

    uint8_t flags = 0;
    if (i->A == 0) {flags |= 0b01000000;} //Z flag
    if (i->A & 0b10000000) {flags |= 0b10000000;} //S flag
    if (!(pairity(i->A) % 2)) {flags |= 0b00000100;} //P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}

enum Error cp_r(struct Instance *__restrict i, void **__restrict inf) {
  i->tmp = i->A;
  uint8_t borrows = 0;
  uint8_t tmp = 0;
  switch (i->instruction & 0b00000111) {
  case A: i->tmp = i->A; break;
  case B: i->tmp = i->B; break;
  case C: i->tmp = i->C; break;
  case D: i->tmp = i->D; break;
  case E: i->tmp = i->E; break;
  case H: i->tmp = i->H; break;
  case L: i->tmp = i->L; break;
  default: return BAD_ARGUMENT;
  }

  borrows = getSubtractBorrows(i->A, i->tmp, 0);
  tmp -= i->tmp;

  uint8_t flags = 0b00000010;
  if (tmp == 0) {flags |= 0b01000000;} //Z flag
  if (tmp & 0b10000000) {flags |= 0b10000000;} //S flag
  if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
  if (borrows & 0b10000000) {flags |= 0b00000001;} // C flag
  if ((i->A ^ i->tmp) & (i->A ^ tmp) & 0x80) {flags |= 0b00000100;} //P/V flag
  i->F = flags;
  return nop(i, inf);
  }  

enum Error cp_n(struct Instance *__restrict i, void **__restrict inf) {
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    *(uint16_t*) inf[0] = i->PC; //addr
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->PC++;
    uint8_t tmp = i->A;
    uint8_t borrows = getSubtractBorrows(i->A, i->tmp, 0);
    tmp -= i->tmp;

    uint8_t flags = 0b00000010;
    if (tmp == 0) {flags |= 0b01000000;} //Z flag
    if (tmp & 0b10000000) {flags |= 0b10000000;} //S flag
    if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
    if (borrows & 0b10000000) {flags |= 0b00000001;} //C flag
    if ((i->A ^ i->tmp) & (i->A ^ tmp) & 0x80) {flags |= 0b00000100;} //P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}  

enum Error cp_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    uint8_t tmp = i->A;
    uint8_t borrows = getSubtractBorrows(i->A, i->tmp, 0);
    tmp -= i->tmp;

    uint8_t flags = 0b00000010;
    if (tmp == 0) {flags |= 0b01000000;} //Z flag
    if (tmp & 0b10000000) {flags |= 0b10000000;} //S flag
    if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
    if (borrows & 0b10000000) {flags |= 0b00000001;} // C flag
    if ((i->A ^ i->tmp) & (i->A ^ tmp) & 0x80) {flags |= 0b00000100;} //P/V flag
    i->F = flags;
    
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}

enum Error inc_r(struct Instance *__restrict i, void **__restrict inf) {
  uint8_t carries = 0;
  uint8_t tmp;
  switch ((i->instruction & 0b00111000) >> 3) {
  case A: tmp = i->A; carries = getAddCarries(i->A, 1 ,0); i->A++; break;
  case B: tmp = i->B; carries = getAddCarries(i->B, 1 ,0); i->B++; break;
  case C: tmp = i->C; carries = getAddCarries(i->C, 1 ,0); i->C++; break;
  case D: tmp = i->D; carries = getAddCarries(i->D, 1 ,0); i->D++; break;
  case E: tmp = i->E; carries = getAddCarries(i->E, 1 ,0); i->E++; break;
  case H: tmp = i->H; carries = getAddCarries(i->H, 1 ,0); i->H++; break;
  case L: tmp = i->L; carries = getAddCarries(i->L, 1 ,0); i->L++; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = i->F & 0b00000001;
  if (tmp + 1 == 0) {flags |= 0b01000000;} //Z flag
  if (tmp + 1 & 0b10000000) {flags |= 0b10000000;} //S flag
  if (carries & 0b00001000) {flags |= 0b00010000;} //H flag
  if (tmp == 0x7F) {flags |= 0b00000100;} // P/V flag
  i->F = flags;
  return nop(i, inf);
}

enum Error inc_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] |= i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->MState = 3;
    i->TCycle = 1;
    uint8_t carries = getAddCarries(i->tmp, 1, 0);
    uint8_t flags = i->F & 0b00000001;
    if (i->tmp + 1 == 0) {flags |= 0b01000000;} //Z flag
    if (i->tmp + 1 & 0b10000000) {flags |= 0b10000000;} //S flag
    if (carries & 0b00001000) {flags |= 0b00010000;} //H flag
    if (i->tmp == 0x7F) {flags |= 0b00000100;} //P/V flag
    i->tmp++;
    i->F = flags;
    
    return SUCCESS;
  }
  if (i->MState == 3) {
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}
enum Error dec_r(struct Instance *__restrict i, void **__restrict inf) {
  uint8_t borrows = 0;
  uint8_t tmp;
  switch ((i->instruction & 0b00111000) >> 3) {
  case A: tmp = i->A; borrows = getSubtractBorrows(i->A, 1 ,0); i->A--; break;
  case B: tmp = i->B; borrows = getSubtractBorrows(i->B, 1 ,0); i->B--; break;
  case C: tmp = i->C; borrows = getSubtractBorrows(i->C, 1 ,0); i->C--; break;
  case D: tmp = i->D; borrows = getSubtractBorrows(i->D, 1 ,0); i->D--; break;
  case E: tmp = i->E; borrows = getSubtractBorrows(i->E, 1 ,0); i->E--; break;
  case H: tmp = i->H; borrows = getSubtractBorrows(i->H, 1 ,0); i->H--; break;
  case L: tmp = i->L; borrows = getSubtractBorrows(i->L, 1 ,0); i->L--; break;
  default: return BAD_ARGUMENT;
  }

  uint8_t flags = i->F & 0b00000001;
  if (tmp - 1 == 0) {flags |= 0b01000000;} //Z flag
  if (tmp - 1 & 0b10000000) {flags |= 0b10000000;} //S flag
  if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
  if (tmp == 0x80) {flags |= 0b00000100;} // P/V flag
  flags |= 0b00000010; //N flag
  i->F = flags;
  return nop(i, inf);
}

enum Error dec_$hl$(struct Instance *__restrict i, void **__restrict inf) {
  if (i->currentOverride && !i->gotDisplacement) {
    return get_displacement(i, inf);
  }
  if (i->MState == 1) {
    i->MState = 2;
    i->TCycle = 1;
    *(uint8_t*) inf[2] = 0; //m1
    if (i->currentOverride == IX_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IX + i->displacement; //addr
    }
    else if (i->currentOverride == IY_OVERRIDE) {
      *(uint16_t*) inf[0] = i->IY + i->displacement; //addr
    }
    else {
      *(uint16_t*) inf[0] = i->H; //addr
      *(uint16_t*) inf[0] = *(uint16_t*) inf[0] << 8; //addr
      *(uint16_t*) inf[0] += i->L; //addr
    }
    return SUCCESS;
  }
  if (i->MState == 2) {
    i->MState = 3;
    i->TCycle = 1;    
    uint8_t borrows = getSubtractBorrows(i->tmp, 1, 0);
    uint8_t flags = i->F & 0b00000001;
    if (i->tmp - 1 == 0) {flags |= 0b01000000;} //Z flag
    if (i->tmp - 1 & 0b10000000) {flags |= 0b10000000;} //S flag
    if (borrows & 0b00001000) {flags |= 0b00010000;} //H flag
    if (i->tmp == 0x7F) {flags |= 0b00000100;} //P/V flag
    flags |= 0b00000010; //N flag
    i->tmp--;
    i->F = flags;
    
    return SUCCESS;
  }
  if (i->MState == 3) {
    return nop(i, inf);
  }
  return BAD_ARGUMENT;
}
