...
enum Error rrca(struct Instance* __restrict i, void** __restrict inf){
  i->F |= 0b00000001 & i->A;
  i->A = i->A >> 1;
...
enum Error rr_r(struct Instance* __restrict i, void** __restrict inf){
  ...
  switch (i->instruction & 0b00000111) {
    ...
    case C:
      ...
      break;
    case D:
      carry = 0b00000001 & i->D;
      i->D = i->L >> 1;
      i->D |= (i->D & 0b00000001) << 7;
      i->F |= carry;
      if (i->D == 0) {i->F |= 0b01000000;} //Z flag
      if (i->D & 0b10000000) {i->F |= 0b10000000;} //S flag
      if (pairity(i->D) % 2 == 0) {i->F |= 0b00000100;} //P/V flag
...
if (i->MState == 1) {
  ...
  if (i->MState == 2) {
    i->F = 0;
    uint8_t carry = 0b00000001 & i->tmp;
    i->tmp = i->tmp >> 1;
    i->tmp |= (i->F & 0b00000001) << 7;
    i->F |= carry;
    if (i->tmp == 0) {i->F |= 0b01000000;} //Z flag
